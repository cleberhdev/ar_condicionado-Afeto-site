    ...
    def validate(self, attrs):
        try:
            ...
            if not PasswordResetTokenGenerator().check_token(user, token):
                raise AuthenticationFailed("O link de mudança de senha é inválido ou expirou.", 401)
            if password != confirm_password:
                raise AuthenticationFailed("Senhão não coicidem!")
            user.set_password(password)
            user.save()
            return attrs
        except Exception as e:
             raise AuthenticationFailed("O link de mudança de senha é inválido ou expirou.")
